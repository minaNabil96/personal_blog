...
function SidebarContent({ collapsed, locale, pathname, isRTL }: {
  collapsed: boolean
  locale: string
  pathname: string
  isRTL: boolean
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-zinc-950 overflow-hidden">
      ...
      {/* Mobile sidebar - slides in from the side */}
      <AnimatePresence>
        {sidebarOpen && (
          <motion.aside
            ...
            className={cn(
              'fixed inset-y-0 z-50 flex flex-col border-r border-zinc-800/50 bg-zinc-900/95 backdrop-blur-xl lg:hidden',
              collapsed ? 'w-16' : 'w-64'
            )}
          >
            <div className="flex items-center justify-end p-2">
              <button
                onClick={() => setSidebarOpen(false)}
                className="rounded-lg p-2 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800/50"
                aria-label="Close menu"
              >
                <X size={20} />
              </button>
            </div>
            <SidebarContent collapsed={false} locale={locale} pathname={pathname} isRTL={isRTL} />
          </motion.aside>
        )}
      </AnimatePresence>
      ...
      <aside
        className={cn(
          'hidden lg:flex flex-col border-r border-zinc-800/50 bg-zinc-900/95 backdrop-blur-xl transition-all duration-300',
          collapsed ? 'w-16' : 'w-64'
        )}
      >
        <SidebarContent collapsed={collapsed} locale={locale} pathname={pathname} isRTL={isRTL} />
      </aside>
